...
public class SwaggerConfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("ssafyVueJS").select()
				.apis(RequestHandlerSelectors.basePackage("com.ssafy.happyhouse.controller")).paths(regex("/vue/.*"))
				.build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("SSAFY HAPPY HOUSE REST API").description(
				"<h2>SSAFY API Reference for Developers</h2>Swagger를 이용한 HAPPY HOUSE API<br><img src=\"/ssafy.png\">\"")
